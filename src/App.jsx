...
async function sbGet(key) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/ayiwoza_kv?key=eq.${encodeURIComponent(key)}&select=value`, {
...
async function sbSet(key, value) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/ayiwoza_kv`, {
    method: "POST",
    headers: sbHeaders,
...
const B = {
  navy:"#1B2D6B", navyDk:"#111E4A", blue:"#1565C0", blueLt:"#1E88E5",
  green:"#2E7D32", gold:"#C9962B", red:"#C0392B",
  bg:"#F5F7FA", border:"#E3E8F0", textMid:"#4A5568", textSub:"#8A96A8"
};
...
// ── Avatar ────────────────────────────────────────────────────────────────────
const Av = ({name,role,photo,size=38}) => (
  photo
    ...
    : <div style={{width:size,height:size,borderRadius:"50%",background:roleColor(role),display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*.3,fontWeight:800,color:"#fff",flexShrink:0,border:"2px solid rgba(201,150,43,.4)"}}>
        {ini(name)}
      </div>
);
...
function Modal({title,onClose,children}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(17,30,74,.5)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
...
const PBtn = ({onClick,children,small=false,col=B.navy}) => (
  <button onClick={onClick} style={{background:`linear-gradient(135deg,${col},${B.blue})`,color:"#fff",border:"none",borderRadius:9,padding:small?"7px 13px":"10px 16px",fontSize:small?12:13,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>{children}</button>
);

const Logo = ({size=36}) => (
  <svg width={size} height={size*.9} viewBox="0 0 120 108" fill="none">
    <circle cx="30" cy="30" r="6" fill="#C9962B"/><path d="M20 55 Q24 38 30 36 Q36 38 40 55" fill="#C9962B"/>
    <circle cx="90" cy="30" r="6" fill="#43A047"/><path d="M80 55 Q84 38 90 36 Q96 38 100 55" fill="#43A047"/>
    ...
    <rect x="54" y="30" width="12" height="38" rx="3" fill="url(#g1)"/><polygon points="60,8 46,30 74,30" fill="url(#g1)"/>
...
const statusStyle = s => ({approved:{bg:"#E8F5E9",c:B.green},rejected:{bg:"#FDECEC",c:B.red},pending:{bg:"#FFF3E0",c:"#E65100"}}[s]||{bg:"#F0F1F3",c:B.textSub});

// ── Photo Picker (fixed: label-based file trigger) ────────────────────────────
...
function AddContribModal({members,contribs,saveC,closeModal,showToast,ctype}) {
  ...
  const [mid,setMid]=useState(""); const [recipientId,setRecipientId]=useState("");
  const [mon,setMon]=useState(""); const [yr,setYr]=useState(new Date().getFullYear());
  const [amt,setAmt]=useState(ct.min);
  const isSocial = ctype==="marriage"||ctype==="naming";
  const save=()=>{
  ...
  return <Modal title={`${ct.icon} Record ${ct.label}`} onClose={closeModal}>
    <Fld label="Contributor (who paid)"><select style={IS} value={mid} onChange={e=>setMid(e.target.value)}>
      <option value="">Select contributor…</option>
      {members.map(m=><option key={m.id} value={m.id}>{m.name} ({m.id})</option>)}
    </select></Fld>
    {isSocial&&<Fld label="Recipient (who is being celebrated)"><select style={IS} value={recipientId} onChange={e=>setRecipientId(e.target.value)}>
      <option value="">Select recipient…</option>
      {members.filter(m=>m.id!==mid).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}
    </select></Fld>}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      <Fld label="Month"><select style={IS} value={mon} onChange={e=>setMon(e.target.value)}>
...
function EditContribModal({record,members,contribs,saveC,closeModal,showToast}) {
  ...
  const isSocial=record.type==="marriage"||record.type==="naming";
  const mid=record.mid||record.contributorId;
  const [recipientId,setRecipientId]=useState(record.recipientId||"");
  const [mon,setMon]=useState(record.month); const [yr,setYr]=useState(record.year);
  const [amt,setAmt]=useState(record.amount);
  const save=()=>{
    if(!mon||!amt){showToast("Fill all fields","error");return;}
    if(isSocial&&!recipientId){showToast("Select recipient","error");return;}
    const updated={...record,month:mon,year:Number(yr),amount:Number(amt)};
    if(isSocial) updated.recipientId=recipientId;
    const nc={...contribs,[mid]:(contribs[mid]||[]).map(c=>c.id===record.id?updated:c)};
    saveC(nc);closeModal();showToast("Updated ✓");
  };
  const del=()=>{
    if(!window.confirm("Delete this record?"))return;
    const nc={...contribs,[mid]:(contribs[mid]||[]).filter(c=>c.id!==record.id)};
    saveC(nc);closeModal();showToast("Deleted","warn");
  };
  return <Modal title={`✏️ Edit ${ct.label}`} onClose={closeModal}>
    {isSocial&&<Fld label="Recipient"><select style={IS} value={recipientId} onChange={e=>setRecipientId(e.target.value)}>
      <option value="">Select recipient…</option>
      {members.filter(m=>m.id!==mid).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}
    </select></Fld>}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      <Fld label="Month"><select style={IS} value={mon} onChange={e=>setMon(e.target.value)}>
        {MONTHS.map(m=><option key={m}>{m}</option>)}</select></Fld>
      <Fld label="Year"><input style={IS} type="number" value={yr} onChange={e=>setYr(e.target.value)}/></Fld>
    </div>
    <Fld label="Amount (₦)"><input style={IS} type="number" value={amt} onChange={e=>setAmt(e.target.value)}/></Fld>
    <div style={{display:"flex",gap:8,marginTop:16}}>
      <button onClick={del} style={{flex:1,background:"transparent",border:`1.5px solid ${B.red}`,borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:700,color:B.red}}>🗑 Delete</button>
...
function EditLoanModal({loan,members,loans,saveL,closeModal,showToast}) {
  ...
  const del=()=>{
    ...
    saveL(loans.filter(l=>l.id!==loan.id));closeModal();showToast("Deleted","warn");
  };
  return <Modal title="✏️ Edit Loan" onClose={closeModal}>
    ...
    <Fld label="Status"><select style={IS} value={status} onChange={e=>setStatus(e.target.value)}>
      <option value="active">Active</option><option value="repaid">Repaid</option>
    </select></Fld>
    <div style={{display:"flex",gap:8,marginTop:16}}>
      <button onClick={del} style={{flex:1,background:"transparent",border:`1.5px solid ${B.red}`,borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:700,color:B.red}}>🗑 Delete</button>
      <button onClick={closeModal} style={{flex:1,background:"transparent",border:`1.5px solid ${B.border}`,borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:700,color:B.textMid}}>Cancel</button>
...
function EditBulkOrderModal({order,bulkOrders,saveB,members,closeModal,showToast}) {
  const [title,setTitle]=useState(order.title); const [notes,setNotes]=useState(order.notes||"");
  ...
  const del=()=>{
    ...
    saveB(bulkOrders.filter(o=>o.id!==order.id));closeModal();showToast("Deleted","warn");
  };
  return <Modal title="✏️ Edit Bulk Order" onClose={closeModal}>
    ...
    {items.map((it,i)=>(
      <div key={i} style={{border:`1.5px solid ${B.border}`,borderRadius:10,padding:10,marginBottom:8}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <Fld label="Item"><select style={IS} value={it.item} onChange={e=>upd(i,"item",e.target.value)}>
            <option value="">Select…</option>{BULK_ITEMS.map(b=><option key={b}>{b}</option>)}</select></Fld>
...
function EditEventPaymentModal({event,payment,events,saveE,members,closeModal,showToast}) {
  ...
  const save=()=>{
    ...
    closeModal();showToast("Payment updated ✓");
  };
  const del=()=>{
  ...
  return <Modal title="✏️ Edit Payment" onClose={closeModal}>
    ...
    <Fld label="Amount (₦)"><input style={IS} type="number" value={amt} onChange={e=>setAmt(e.target.value)}/></Fld>
    <div style={{display:"flex",gap:8,marginTop:16}}>
      <button onClick={del} style={{flex:1,background:"transparent",border:`1.5px solid ${B.red}`,borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:700,color:B.red}}>🗑 Remove</button>
      <button onClick={closeModal} style={{flex:1,background:"transparent",border:`1.5px solid ${B.border}`,borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:700,color:B.textMid}}>Cancel</button>
      <button onClick={save} style={{flex:2,background:`linear-gradient(135deg,${B.navy},${B.blue})`,color:"#fff",border:"none",borderRadius:9,padding:11,cursor:"pointer",fontSize:13,fontWeight:800}}>Save</button>
    </div>
  </Modal>;
...
function AddLoanModal({members,loans,saveL,closeModal,showToast,loanPool,totOut,getOut,preselect=null}) {
  ...
  return <Modal title="🏦 Approve Loan" onClose={closeModal}>
    <div style={{background:"#FFF3E0",borderRadius:9,padding:"10px 12px",marginBottom:12,fontSize:12,color:"#E65100",border:"1px solid #FFE0B2"}}>
      Available: <strong style={{color:B.green}}>₦{fmt(available)}</strong> · 5% admin charge applies
    </div>
    <Fld label="Member"><select style={IS} value={mid} onChange={e=>setMid(e.target.value)}>
      <option value="">Select…</option>
      {members.filter(m=>getOut(m.id)===0).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}
    </select></Fld>
    ...
    {amt>0&&<div style={{background:"#E8F5E9",borderRadius:9,padding:10,fontSize:12,marginBottom:4,border:"1px solid #C8E6C9"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,textAlign:"center"}}>
        {[["5% Charge",`₦${fmt(amt*.05)}`,"#7A5800"],["Receives",`₦${fmt(amt-amt*.05)}`,B.green],["Repays",`₦${fmt(amt)}`,B.navy]].map(([l,v,c])=>(
          <div key={l} style={{background:"#fff",borderRadius:7,padding:8}}><p style={{margin:0,fontSize:10,color:B.textSub}}>{l}</p><p style={{margin:"3px 0 0",fontWeight:800,color:c,fontSize:12}}>{v}</p></div>))}
      </div>
    </div>}
...
function LoanPayModal({loan,members,loans,saveL,closeModal,showToast}) {
  ...
  return <Modal title="💳 Loan Payment" onClose={closeModal}>
...
function NewEventModal({etype,members,events,saveE,closeModal,showToast,userName}) {
  const [name,setName]=useState(""); const [date,setDate]=useState("");
  const perHead=etype==="marriage"?5000:500;
  const save=()=>{
    if(!name){showToast("Enter event name","error");return;}
    saveE([...events,{id:Date.now(),etype,name,date:date||today(),payments:[]}]);
    closeModal();showToast("Event created ✓");
  };
  return <Modal title={`${etype==="marriage"?"💍":"👶"} New ${etype==="marriage"?"Marriage":"Naming Ceremony"}`} onClose={closeModal}>
    <Fld label="Event Name"><input style={IS} value={name} onChange={e=>setName(e.target.value)} placeholder={`e.g. ${etype==="marriage"?"Marriage":"Naming"} of ${userName}`}/></Fld>
    <Fld label="Date"><input style={IS} type="date" value={date} onChange={e=>setDate(e.target.value)}/></Fld>
    <div style={{background:"#E8F5E9",borderRadius:9,padding:10,fontSize:12,border:"1px solid #C8E6C9"}}>
      Each member contributes: <strong style={{color:B.green}}>₦{fmt(perHead)}</strong><br/>
      Total expected: <strong style={{color:B.navy}}>₦{fmt(perHead*members.length)}</strong>
    </div>
...
function EventPayModal({event,member,events,saveE,closeModal,showToast}) {
  const perHead=event.etype==="marriage"?5000:500;
  const [amt,setAmt]=useState(perHead);
  const save=()=>{
  ...
  return <Modal title="💳 Record Payment" onClose={closeModal}>
    <div style={{background:B.bg,borderRadius:9,padding:12,marginBottom:12,border:`1px solid ${B.border}`}}>
...
function AddBulkModal({members,bulkOrders,saveB,closeModal,showToast}) {
  ...
  const [items,setItems]=useState([{item:"",total:"",qty:""}]);
  const upd=(i,k,v)=>setItems(items.map((it,idx)=>idx===i?{...it,[k]:v}:it));
  const save=()=>{
  ...
  return <Modal title="🛒 New Bulk Purchase" onClose={closeModal}>
    ...
    {items.map((it,i)=>(
      <div key={i} style={{border:`1.5px solid ${B.border}`,borderRadius:10,padding:10,marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:11,fontWeight:700,color:B.textSub}}>Item {i+1}</span>
          {items.length>1&&<button onClick={()=>setItems(items.filter((_,idx)=>idx!==i))} style={{background:"transparent",border:"none",color:B.red,cursor:"pointer",fontSize:13,fontWeight:800}}>✕</button>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <Fld label="Item"><select style={IS} value={it.item} onChange={e=>upd(i,"item",e.target.value)}>
            <option value="">Select…</option>{BULK_ITEMS.map(b=><option key={b}>{b}</option>)}</select></Fld>
...
function AddMemberModal({members,saveM,closeModal,showToast}) {
  ...
  const [adm,setAdm]=useState(false); const [photo,setPhoto]=useState(null);
  const save=()=>{
    if(!name.trim()||!phone||!pw){showToast("Fill all fields","error");return;}
    const newId="AYI"+String(members.length+1).padStart(4,"0");
    saveM([...members,{id:newId,name:name.toUpperCase().trim(),role,phone,joined:new Date().toLocaleDateString("en-NG",{month:"long",year:"numeric"}),password:pw,isAdmin:adm,photo}]);
    closeModal();showToast("Member added ✓");
  };
  return <Modal title="👤 Add Member" onClose={closeModal}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      <Fld label="Role"><select style={IS} value={role} onChange={e=>setRole(e.target.value)}>
        {["Member","Deputy Chairman","Secretary","Treasurer"].map(r=><option key={r}>{r}</option>)}
      </select></Fld>
    ...
    <Fld label="Admin Access"><select style={IS} value={String(adm)} onChange={e=>setAdm(e.target.value==="true")}>
...
function MemberDetailModal({member,members,saveM,closeModal,showToast,isAdmin,getMT,getMC,getOut,pShare}) {
  ...
  const savePhoto=()=>{
    saveM(members.map(m=>m.id===member.id?{...m,photo}:m));
    setEditPhoto(false);showToast("Photo updated ✓");
  };
  return <Modal title="👤 Member Profile" onClose={closeModal}>
    ...
    {editPhoto&&<div style={{marginBottom:14,padding:14,background:"#F0F4FF",borderRadius:10,border:`1.5px dashed ${B.blue}`}}>
    ...
    {isAdmin&&<button onClick={()=>{if(window.confirm(`Remove ${member.name}?`)){saveM(members.filter(x=>x.id!==member.id));closeModal();showToast("Removed","warn");}}} style={{width:"100%",marginTop:14,background:"transparent",border:`1.5px solid ${B.red}`,borderRadius:9,padding:10,cursor:"pointer",fontSize:12,fontWeight:700,color:B.red}}>🗑 Remove Member</button>}
...
function ChangePwModal({user,members,saveM,setUser,closeModal,showToast}) {
  ...
  return <Modal title="🔑 Change Password" onClose={closeModal}>
...
function LoanApplyModal({user,loanApplications,saveLoanApps,closeModal,showToast,getOut,loanPool,totOut}) {
  ...
  const save=()=>{
    ...
    if(hasPending){showToast("You have a pending application","warn");return;}
  ...
  return <Modal title="🏦 Apply for Loan" onClose={closeModal}>
    {hasActive&&<div style={{background:"#FDECEC",borderRadius:9,padding:10,marginBottom:12,fontSize:12,color:B.red,border:"1px solid #FFCDD2"}}>⚠ You have an active loan. Clear it first.</div>}
    {hasPending&&<div style={{background:"#FFF3E0",borderRadius:9,padding:10,marginBottom:12,fontSize:12,color:"#E65100",border:"1px solid #FFE0B2"}}>⚠ You already have a pending application.</div>}
    <div style={{background:"#E8F5E9",borderRadius:9,padding:10,marginBottom:12,fontSize:12,border:"1px solid #C8E6C9"}}>
      Max available: <strong style={{color:B.green}}>₦{fmt(available)}</strong> · 5% admin charge
    </div>
    ...
    {amt>0&&!hasActive&&<div style={{background:B.bg,borderRadius:9,padding:10,fontSize:12,marginBottom:4,border:`1px solid ${B.border}`}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,textAlign:"center"}}>
        {[["5% Charge",`₦${fmt(amt*.05)}`,"#7A5800"],["You Receive",`₦${fmt(amt-amt*.05)}`,B.green],["You Repay",`₦${fmt(amt)}`,B.navy]].map(([l,v,c])=>(
          <div key={l} style={{background:"#fff",borderRadius:7,padding:8}}><p style={{margin:0,fontSize:10,color:B.textSub}}>{l}</p><p style={{margin:"2px 0 0",fontWeight:800,color:c,fontSize:12}}>{v}</p></div>))}
      </div>
    </div>}
...
function BulkRequestModal({user,bulkRequests,saveBulkReqs,closeModal,showToast}) {
  ...
  const upd=(i,k,v)=>setItems(items.map((it,idx)=>idx===i?{...it,[k]:v}:it));
  const addRow=()=>{ if(items.length<5) setItems([...items,{item:"",qty:"",notes:""}]); };
  const removeRow=i=>{ if(items.length>1) setItems(items.filter((_,idx)=>idx!==i)); };
  const save=()=>{
    const valid=items.filter(it=>it.item&&it.qty);
    if(!valid.length){showToast("Add at least one item with qty","error");return;}
  ...
  return <Modal title="🛒 Request Bulk Items" onClose={closeModal}>
    <div style={{background:"#E8F4FF",borderRadius:9,padding:10,marginBottom:12,fontSize:12,color:B.blue,border:`1px solid ${B.blueLt||"#90CAF9"}`}}>
      Select up to <strong>5 items</strong> for the next bulk purchase.
    </div>
    {items.map((it,i)=>(
      <div key={i} style={{border:`1.5px solid ${B.border}`,borderRadius:10,padding:10,marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:11,fontWeight:700,color:B.textSub}}>Item {i+1}</span>
          {items.length>1&&<button onClick={()=>removeRow(i)} style={{background:"transparent",border:"none",color:B.red,cursor:"pointer",fontSize:14,fontWeight:800,lineHeight:1}}>✕</button>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <Fld label="Item"><select style={IS} value={it.item} onChange={e=>upd(i,"item",e.target.value)}>
            <option value="">Select…</option>{BULK_ITEMS.map(b=><option key={b}>{b}</option>)}</select></Fld>
        ...
        <Fld label="Notes (optional)"><input style={IS} value={it.notes} onChange={e=>upd(i,"notes",e.target.value)} placeholder="Brand or preference…"/></Fld>
      </div>))}
    {items.length<5&&<button onClick={addRow} style={{width:"100%",background:B.bg,border:`1.5px dashed ${B.border}`,borderRadius:9,padding:9,cursor:"pointer",fontSize:13,fontWeight:700,color:B.navy,marginBottom:12}}>+ Add Item ({items.length}/5)</button>}
...
function ProfileModal({user,setUser,members,saveM,closeModal,showToast}) {
  ...
  const savePhoto=()=>{
    saveM(members.map(m=>m.id===user.id?{...m,photo}:m));
    setUser({...user,photo});setChanged(false);showToast("Photo updated ✓");
  };
  const saveEmail=()=>{
  ...
  return <Modal title="👤 My Profile" onClose={closeModal}>
    ...
    <div style={{background:B.bg,borderRadius:10,padding:14,border:`1px solid ${B.border}`,marginBottom:editEmail?8:14}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0"}}>
        ...
        {!editEmail&&<div style={{display:"flex",alignItems:"center",gap:6}}>
    ...
    {editEmail&&<div style={{marginBottom:14}}>
...
function AppInner() {
  ...
  const getGiftsReceived=(uid,type)=>{
    return Object.values(contribs).flat().filter(c=>c.type===type&&c.recipientId===uid);
  };

  const pendingLoans=loanApplications.filter(a=>a.status==="pending");
  const pendingBulk=bulkRequests.filter(r=>r.status==="pending");

  if(!ready) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:B.bg,flexDirection:"column",gap:16}}><Logo size={72}/><p style={{color:B.navy,fontWeight:700}}>Loading…</p></div>;
  ...
  const approveBulkReq=r=>{saveBulkReqs(bulkRequests.map(x=>x.id===r.id?{...x,status:"approved"}:x));showToast("Approved ✓");};
  const rejectBulkReq=r=>{saveBulkReqs(bulkRequests.map(x=>x.id===r.id?{...x,status:"rejected"}:x));showToast("Rejected","warn");};

  const renderModal=()=>{
    if(!modal)return null;
    const {type,data={}}=modal;
    const c={closeModal,showToast};
    ...
    if(type==="newEvent")     return <NewEventModal {...c} etype={data.etype} members={members} events={events} saveE={saveE} userName={user.name.split(" ")[0]}/>;
    if(type==="eventPay")     return <EventPayModal {...c} event={data.event} member={data.member} events={events} saveE={saveE}/>;
    if(type==="editEventPay") return <EditEventPaymentModal {...c} event={data.event} payment={data.payment} events={events} saveE={saveE} members={members}/>;
  ...
  const renderPage=()=>{
    ...
    if(view==="contributions") {
      ...
      const isSocial=ctab==="marriage"||ctab==="naming";

      let rows=[];
      if(isAdmin) {
        // admin sees all contributor records
        rows=Object.entries(contribs).flatMap(([mid,cs])=>
          cs.filter(c=>c.type===ctab).map(c=>({
            ...c, mid,
            contributorName: members.find(m=>m.id===mid)?.name||mid,
            recipientName: c.recipientId ? members.find(m=>m.id===c.recipientId)?.name||c.recipientId : null,
          }))).sort((a,b)=>b.id-a.id);
      } else if(isSocial) {
        // non-admin sees gifts received by themselves
        rows=getGiftsReceived(user.id,ctab).map(c=>({
          ...c,
          contributorName: members.find(m=>m.id===c.contributorId||m.id===c.mid)?.name||"Unknown",
          recipientName: user.name,
        })).sort((a,b)=>b.id-a.id);
      } else {
        // savings / monthly: member sees their own payments
        rows=getMC(user.id,ctab).map(c=>({...c,mid:user.id})).sort((a,b)=>b.id-a.id);
      }

      const totalShown=rows.reduce((s,c)=>s+Number(c.amount),0);
      ...
      return <>
        ...
        {/* context banner */}
        {!isAdmin&&isSocial&&<div style={{background:"#E8F4FF",borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:B.blue,border:`1px solid #90CAF9`}}>
          Showing <strong>gifts received by you</strong> for your {ctab==="marriage"?"marriage":"naming ceremony"}.
        </div>}

        <Card>
          <SecHead title={`${ct.icon} ${ct.label}`} right={<div style={{display:"flex",alignItems:"center",gap:8}}>
            <strong style={{color:ct.color}}>₦{fmt(isAdmin?totC(ctab):totalShown)}</strong>
            {isAdmin&&<PBtn small onClick={()=>setModal({type:"addContrib",data:{ctype:ctab}})}>+ Record</PBtn>}
          </div>}/>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
              <thead><tr>
                {/* admin: contributor col */}
                {isAdmin&&<th style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg,whiteSpace:"nowrap"}}>Contributor</th>}
                {/* admin social: recipient col */}
                {isAdmin&&isSocial&&<th style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg,whiteSpace:"nowrap"}}>Recipient</th>}
                {/* member social: contributor col */}
                {!isAdmin&&isSocial&&<th style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg,whiteSpace:"nowrap"}}>From</th>}
                {["Period","Amount","Date"].map(h=><th key={h} style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg,whiteSpace:"nowrap"}}>{h}</th>)}
                {isAdmin&&<th style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg}}>Edit</th>}
              </tr></thead>
              <tbody>
                {rows.length===0&&<tr><td colSpan={7} style={{textAlign:"center",padding:24,color:B.textSub}}>
                  {!isAdmin&&isSocial?"No gifts received yet.":"No records yet."}
                </td></tr>}
                {rows.map(c=>(
                  <tr key={c.id} onClick={()=>isAdmin&&setModal({type:"editContrib",data:{record:c}})} style={{cursor:isAdmin?"pointer":"default"}}>
                    ...
                    {isAdmin&&isSocial&&<td style={{padding:"9px 10px",borderBottom:`1px solid ${B.border}`,fontSize:11,fontWeight:600}}>{(c.recipientName||"-").split(" ")[0]}</td>}
                    {!isAdmin&&isSocial&&<td style={{padding:"9px 10px",borderBottom:`1px solid ${B.border}`,fontSize:11,fontWeight:600}}>{(c.contributorName||"").split(" ")[0]}</td>}
        ...
        {isAdmin&&isSocial&&<Card>
          <SecHead title="Gifts by Recipient"/>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
              <thead><tr>{["Recipient","Total Received","Count"].map(h=><th key={h} style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg}}>{h}</th>)}</tr></thead>
              <tbody>{members.map(m=>{
                const gifts=getGiftsReceived(m.id,ctab);
                ...
                return <tr key={m.id}>
                  ...
                  <td style={{padding:"9px 10px",borderBottom:`1px solid ${B.border}`}}><strong style={{color:ct.color}}>₦{fmt(total)}</strong></td>
                  <td style={{padding:"9px 10px",borderBottom:`1px solid ${B.border}`}}>{gifts.length}x</td>
                </tr>;
              })}</tbody>
            </table>
          </div>
    ...
    // ── SOCIAL (event-based contributions)
    if(view==="social") {
      const evList=events.filter(e=>e.etype===stab);
      const perHead=stab==="marriage"?5000:500;
      return <>
        <div style={{display:"flex",gap:6,marginBottom:12}}>
          {[["marriage","💍 Marriage"],["naming","👶 Naming"]].map(([k,l])=>(
            <button key={k} onClick={()=>setStab(k)} style={{flex:1,padding:"8px",borderRadius:20,border:`1.5px solid ${stab===k?B.navy:B.border}`,background:stab===k?B.navy:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",color:stab===k?"#fff":B.textSub}}>{l}</button>))}
        </div>
        {isAdmin&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}>
          <PBtn small onClick={()=>setModal({type:"newEvent",data:{etype:stab}})}>+ New Event</PBtn>
        </div>}
        {evList.length===0&&<Card><div style={{padding:30,textAlign:"center",color:B.textSub}}>No {stab} events yet.</div></Card>}
        {[...evList].reverse().map(ev=>{
          const collected=ev.payments.reduce((s,p)=>s+Number(p.amount),0);
          const expected=perHead*members.length;
          // members only see their own payment status in events
          const memberList=isAdmin?members:[members.find(m=>m.id===user.id)].filter(Boolean);
          return <Card key={ev.id} mb={12}>
            <div style={{padding:"12px 14px",background:B.bg,borderBottom:`1.5px solid ${B.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{textAlign:"right"}}>
                {isAdmin&&<p style={{margin:0,fontWeight:800,color:B.green,fontSize:13}}>₦{fmt(collected)} / ₦{fmt(expected)}</p>}
                <p style={{margin:0,fontSize:10,color:B.textSub}}>{ev.payments.length}/{members.length} paid</p>
              </div>
            </div>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
                <thead><tr>{["Member","Status","Date",isAdmin?"Pay":""].filter(Boolean).map(h=><th key={h} style={{textAlign:"left",padding:"7px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg}}>{h}</th>)}</tr></thead>
                <tbody>{memberList.map(m=>{
                  const paid=ev.payments.find(p=>p.memberId===m.id);
                  return <tr key={m.id} onClick={()=>isAdmin&&paid&&setModal({type:"editEventPay",data:{event:ev,payment:paid}})} style={{cursor:isAdmin&&paid?"pointer":"default"}}>
                    <td style={{padding:"8px 10px",borderBottom:`1px solid ${B.border}`,fontSize:11,fontWeight:600}}>{m.name.split(" ")[0]}</td>
                    <td style={{padding:"8px 10px",borderBottom:`1px solid ${B.border}`}}>{paid?<Badge text={`₦${fmt(paid.amount)}${isAdmin?" ✏️":""}`} color={B.green} bg="#E8F5E9"/>:<Badge text="Pending" color="#E65100" bg="#FFF3E0"/>}</td>
                    <td style={{padding:"8px 10px",borderBottom:`1px solid ${B.border}`,fontSize:10}}>{paid?paid.date:"-"}</td>
                    {isAdmin&&<td style={{padding:"8px 10px",borderBottom:`1px solid ${B.border}`}}>
    ...
    if(view==="loans") {
      ...
      return <>
        ...
        <Card>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
              <thead><tr>{["Member","Amount","Balance","Status",isAdmin?"Pay":""].filter(Boolean).map(h=><th key={h} style={{textAlign:"left",padding:"8px 10px",color:B.textSub,fontWeight:700,fontSize:10,textTransform:"uppercase",borderBottom:`1.5px solid ${B.border}`,background:B.bg,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
              <tbody>
                ...
                {[...visLoans].reverse().map(l=>{
                  ...
                  return <tr key={l.id} onClick={()=>isAdmin&&setModal({type:"editLoan",data:{loan:l}})} style={{cursor:isAdmin?"pointer":"default"}}>
        ...
        {[...bulkOrders].reverse().map(o=>(
          <div key={o.id} onClick={()=>isAdmin&&setModal({type:"editBulkOrder",data:{order:o}})} style={{padding:"12px 14px",borderBottom:`1px solid ${B.border}`,cursor:isAdmin?"pointer":"default"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <strong style={{fontSize:13,color:B.navy}}>{o.title} {isAdmin&&<span style={{fontSize:11}}>✏️</span>}</strong>
      ...
      return <>
        ...
        <Card mb={14}>
          ...
          {[...myLoanApps].reverse().map(a=>{
            ...
            return <div key={a.id} style={{padding:"11px 14px",borderBottom:`1px solid ${B.border}`}}>
        ...
        <Card mb={14}>
          ...
          {[...myBulkReqs].reverse().map(r=>{
            ...
            return <div key={r.id} style={{padding:"11px 14px",borderBottom:`1px solid ${B.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                <strong style={{color:B.navy,fontSize:13}}>{r.items?.length||0} item(s) requested</strong>
            ...
            {[...loanApplications].reverse().map(a=>(
              <div key={a.id} style={{padding:"12px 14px",borderBottom:`1px solid ${B.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <Badge text={a.status.toUpperCase()} color={statusStyle(a.status).c} bg={statusStyle(a.status).bg}/>
                </div>
                {a.status==="pending"&&<div style={{display:"flex",gap:8,marginTop:9}}>
            ...
            {[...bulkRequests].reverse().map(r=>(
              <div key={r.id} style={{padding:"12px 14px",borderBottom:`1px solid ${B.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <Badge text={r.status.toUpperCase()} color={statusStyle(r.status).c} bg={statusStyle(r.status).bg}/>
                </div>
                {r.status==="pending"&&<div style={{display:"flex",gap:8,marginTop:9}}>
                  <button onClick={()=>approveBulkReq(r)} style={{flex:1,background:B.green,color:"#fff",border:"none",borderRadius:8,padding:"7px",cursor:"pointer",fontSize:12,fontWeight:800}}>✓ Approve</button>
                  <button onClick={()=>rejectBulkReq(r)} style={{flex:1,background:B.red,color:"#fff",border:"none",borderRadius:8,padding:"7px",cursor:"pointer",fontSize:12,fontWeight:800}}>✗ Reject</button>
                </div>}
              </div>))}
          </Card>
        </>}
    ...
    if(view==="reports") {
      ...
      return <>
        ...
        <Card mb={12}>
          <SecHead title="All Contributions"/>
          {CTYPES.map(ct=>(<div key={ct.key} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",borderBottom:`1px solid ${B.border}`}}>
            <span style={{fontSize:12,color:B.textMid}}>{ct.icon} {ct.label}</span>
            <strong style={{color:ct.color,fontSize:13}}>₦{fmt(totC(ct.key))}</strong>
          </div>))}
      ...
      <div style={{background:`linear-gradient(135deg,${B.navyDk},${B.blue})`,position:"sticky",top:0,zIndex:50,boxShadow:"0 3px 16px rgba(17,30,74,.35)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <Logo size={34}/>
            <div><p style={{fontSize:12,fontWeight:900,color:"#fff",letterSpacing:1,margin:0}}>AYIWOZA</p><p style={{fontSize:8,color:"#E0B84A",margin:0}}>COMMITTEE OF FRIENDS</p></div>
          </div>
        ...
        <div style={{display:"flex",overflowX:"auto",padding:"0 6px 8px"}}>
          {NAVS.map(n=>(
            <button key={n.k} onClick={()=>setView(n.k)} style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"5px 10px",borderRadius:8,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:view===n.k?"rgba(255,255,255,.18)":"transparent",color:view===n.k?"#E0B84A":"rgba(255,255,255,.65)",borderBottom:view===n.k?"2px solid #E0B84A":"2px solid transparent",position:"relative"}}>
              ...
              {n.k==="applications"&&isAdmin&&(pendingLoans.length+pendingBulk.length)>0&&<span style={{position:"absolute",top:2,right:4,background:B.red,color:"#fff",fontSize:8,fontWeight:900,borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center"}}>{pendingLoans.length+pendingBulk.length}</span>}
            </button>))}
        </div>
      </div>
...
function LoginPage({members,onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#111E4A 0%,#1B2D6B 45%,#1565C0 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{background:"#fff",borderRadius:20,padding:24,width:"100%",maxWidth:360,boxShadow:"0 30px 80px rgba(0,0,0,.4)"}}>
